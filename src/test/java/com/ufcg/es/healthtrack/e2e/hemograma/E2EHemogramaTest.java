...
public class E2EHemogramaTest {
    ...
    public void éTentadoCadastrarUmNovoExameDeGlicemia() {
        response = given().
                ...
                body(new HemogramaDTO("des", LocalDateTime.now(),1,1,1,1,1,1,1,1)).port(port).
                when().post("api/v1/exame/hemograma");
    }
    ...
    public void éTentadoListarTodosOsExames() {
        response = given().
                ...
                when().get("api/v1/exame/hemograma/listar");
    }
    ...
    public void éTentadoVisualizarTodosOsExames(String id) {
        response = given().
                header(new Header("Authorization", token)).
                port(port).
                when().get("api/v1/exame/hemograma/"+id);
    }

    @And("Existe um exame cadastrado")
    public void existeUmExameCadastrado() {
        HemogramaDTO exame = new HemogramaDTO("des", LocalDateTime.now(),1,1,1,1,1,1,1,1);
