...
public class E2EFezesTest {
    ...
    public void éTentadoCadastrarUmNovoExameDeGlicemia() {
        response = given().
                ...
                body(new FezesDTO("desc", LocalDateTime.now(),"a","a","a","a","a")).port(port).
                when().post("api/v1/exame/fezes");
    }
    ...
    public void éTentadoListarTodosOsExames() {
        response = given().
                ...
                when().get("api/v1/exame/fezes/listar");
    }
    ...
    public void éTentadoVisualizarTodosOsExames(String id) {
        response = given().
                header(new Header("Authorization", token)).
                port(port).
                when().get("api/v1/exame/fezes/"+id);
    }

    @And("Existe um exame cadastrado")
    public void existeUmExameCadastrado() {
        FezesDTO exame = new FezesDTO("desc", LocalDateTime.now(),"a","a","a","a","a");
